...
def action():
    ...
    noise_values1 = tuple(linear_values1.ys[i] + random_values1.ys[i] for i in range(len(linear_values1.ys)))
    noise_values2 = tuple(linear_values2.ys[i] * random_values2.ys[i] for i in range(len(linear_values2.ys)))
...
def random():
    n = 10000
    random_values = calculator.calculate_random_trend(n, -5, 5)
    ui_controller.lt_panel.plot_graph(random_values.xs, random_values.ys, 'Standard random [-5, 5]')

    random_values = calculator.calculate_custom_random_trend(n, -5, 5)
