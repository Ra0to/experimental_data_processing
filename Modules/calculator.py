...
def calculate_usa_stock_market_trend(trend: UsaStockMarketTrend) -> PlotData:
    xs = []
    ys = []
    for i in range(trend.N):
        t = i * trend.dt
        xs.append(t)
        m = trend.a * trend.c + t * math.exp(trend.b * t) + trend.d * math.sin(t / trend.d) * random.uniform(0, trend.c)
        ys.append(m)
